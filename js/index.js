...
zapS.addEventListener("input", function() {
    const level = parseInt(zapS.value, 10);
    const dmg = getSpellDamage("Lightning", level);
    dmgP.textContent = `Level: ${level} Damage: ${dmg}`; 
});

eqs.addEventListener("input", function() {
    const level = parseInt(eqs.value, 10);
    const dmg = getSpellDamage("earthquake", level);
    dmgE.textContent = `Level: ${level} Damage: ${dmg}`;
});

gitanarrows.addEventListener("input", function() {
    const level = parseInt(gitanarrows.value, 10); 
    const dmg = getSpellDamage("giantarrow", level);
    dmgA.textContent = `Level: ${level} Damage: ${dmg}`;
});

fireballS.addEventListener("input", function() {
    const level = parseInt(fireballS.value, 10);
    const dmg = getSpellDamage("fireball", level);
    dmgF.textContent = `Level: ${level} Damage: ${dmg}`;
});